# Public Wallets and Transactions

## Use of This File

This is a seed list for reconstructing the public Bybit/Lazarus trace.

The addresses below come from public government reporting, victim reporting, technical incident analysis, or public blockchain explorer data. They should not all be treated as the same type of evidence.

The source and evidence level matter.

---

# Primary Incident Addresses
...
**Role:** Compromised Bybit Ethereum multisig cold wallet  
**Evidence:** VERIFIED ON-CHAIN / OFFICIAL VICTIM REPORTING

---

## Initial Attack Operator

**Address**

`0x0fa09C3A328792253f8dee7116848723b72a6d2e`

**Role:** Public technical reporting identifies this address as executing the initial malicious transaction.  
**Evidence:** VERIFIED ON-CHAIN / SUPPORTED ATTRIBUTION
...
**Role:** Malicious Safe implementation containing the backdoor functions used to drain assets.  
**Evidence:** VERIFIED ON-CHAIN / SUPPORTED ATTRIBUTION
...
**Role:** Contract used in the malicious delegate-call / implementation-change sequence.  
**Evidence:** VERIFIED ON-CHAIN / SUPPORTED ATTRIBUTION
...
Public technical analysis identifies this as the transaction that changed the Safe implementation to the malicious contract.
...
**Reported amount:** approximately 401,346 ETH
...
**Reported amount:** 90 USDT

---

# FBI TraderTraitor Ethereum Addresses

On February 26, 2025, the FBI published the following Ethereum addresses in connection with the Bybit theft. The FBI stated that these addresses were holding or had held assets from the theft and were operated by or closely connected to North Korean TraderTraitor actors.

For this case study these are treated as **OFFICIAL ATTRIBUTION** seeds. The inclusion of an address in this list does not by itself establish which exact Bybit transaction reached that address. That still has to be established through the blockchain trace.
...
Do not put every address into the graph and assume the result is the case.

A cleaner reconstruction is:

1. Start with the compromised Bybit cold wallet.
2. Confirm the exploit transactions.
3. Identify the first receiving addresses for each stolen asset.
4. Trace the ETH and converted ERC-20 value forward.
5. Use the FBI list as known-attribution checkpoints.
6. Confirm which FBI addresses actually intersect the branch being reconstructed.
7. Mark DEX, bridge, mixer, swap-service, exchange, and OTC nodes separately from normal self-hosted wallets.
8. Preserve full transaction hashes for every material hop.
9. Split the final graph into phases rather than one uncontrolled cluster.

---

# Important Limitation

The FBI list is an attribution list, not a transaction-by-transaction answer key. A proper reconstruction still requires confirming how the stolen Bybit assets moved into, through, or around each relevant address.
